...
def save_to_excel(wallets1: list, wallets2: list) -> None:
    ...
    worksheet.write(len(wallets1) + 3, 1, '0x2e69Da32b0F7e75549F920CD2aCB0000000F0E7')
...
def update_database() -> None:
    ...
    while True:
        try:
            ...
            if (response.status_code == 200):
                logger.success(f'База данных №{QUERY1} успешно скачан!')
                break
            else:
                logger.error(f'Ошибка обновления базы данных: {response.text} | Cтатус запроса: {response.status_code}')
        except Exception as error:
    ...
    while True:
        try:
            ...
            if (response.status_code == 200):
                ...
                logger.error(f'Ошибка базы данных: {response.text} | Cтатус запроса: {response.status_code}')
        except Exception as error:
